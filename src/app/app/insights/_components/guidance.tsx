...
function buildTips(snap: HistorySummaryOutput): string[] {
  ...
  if (worstPair && worstPair.weakness > 0) {
    ...
    tips.push(
      slowPct != null
        ? `Your slowest letter pair is "${worstPair.key}" — typed ${slowPct}% slower than your overall baseline. The Weakness gauntlet drill at /app/drills bursts a passage built around it.`
        : `Your slowest letter pair is "${worstPair.key}". The Weakness gauntlet drill at /app/drills burst-trains it under sudden death.`,
    );
  }
