...
/** Single-line dropdown picker for race modes. Replaces the previous
 *  stacked chip list so the rail stays compact as more modes land —
 *  every new mode is one more menu item, not another row competing
 *  with YOUR RUN / RACE FEED for vertical space. */
function ModePicker({
  modeId,
  onPick,
  disabled,
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild disabled={disabled}>
        <button
          ...
          className={cn(
            "group flex w-full items-center justify-between gap-3 rounded-md border border-border bg-card px-3 py-2 text-left transition-colors",
