...
export const submit = defineRoute<SubmitTestInput, SubmitTestOutput>({
  ...
  handler: async ({ input, db, meta, log }) => {
    ...
    if (isPersonalBest) {
      ...
      // Fan the PB out to the user's *friends* (mutual follows) — "a
      // friend hit a PB", the social half of the activity feed. "Friend"
      // is the mutual relationship everywhere else (it gates invites +
      // spectate), so a one-way follower the typist has never followed
      // back must NOT get a notification claiming friendship (FT-051).
      // Mutuals also bound the fan-out far tighter than every follower.
      // Its own try so a Clerk hiccup, zero friends, or anything else
      // here can never turn a good submit into a 5xx. Deduped on the run
      // id so a friend gets at most one row per PB.
      try {
