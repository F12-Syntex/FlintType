...
| App shell with sidebar    | `grid h-full grid-cols-1 lg:grid-cols-[var(--app-rail-width)_1fr] lg:gap-3` — used by `/leaderboard`. `--app-rail-width` lives in `globals.css`; the `<TopBar>` reuses the same grid template (`[var(--app-rail-width)_1fr_auto]` with matching `lg:px-6`) so the logo column aligns with the sidebar and the nav aligns with the content card. |
| App shell + preview rail  | `grid h-full grid-cols-1 lg:grid-cols-[var(--app-rail-width)_minmax(0,1fr)_22rem] lg:gap-3` — `/customise` adds a third column: a sticky `<LivePreview>` rail (§12.5). The middle column owns the scroll; the rail is its own scroll-independent column so it stays pinned. Below `lg` the rail collapses and the preview rides as a `sticky top-0` strip at the top of the content scroller. |
...
Sections carry **no** per-section preview card. Instead a single live preview rail (§12.5) sits beside the whole page and reflects every override against the real practice surface — so the rows stay a dense, scannable list and the user still sees exactly how a choice renders.
...
### 12.5 Sections are anchors on one page — one shared live preview rail
...
**One shared live preview rail, never per-section cards.** A single `<LivePreview>` (`customise/_components/live-preview.tsx`) shows the real practice surface — the actual `<Passage>`, `<MobileReadouts>`, and `<Keyboard>` mounted read-only inside a frozen `<PreviewPracticeProvider>` (`customise/_components/preview-practice.tsx`) — and repaints the instant any pref changes, because those are the same components `/app` runs (no parallel rendering to drift). `customise/layout.tsx` places it **once**: a sticky right-hand rail at `lg+` (its own scroll-independent column, so it stays pinned while the options scroll) and a sticky strip atop the content scroller below `lg` (`compact`, no keyboard, so it doesn't swallow a phone viewport). This is the decisive "how will it render" answer.

What it does **not** do: mount a preview *per section*. An earlier revision shipped a bespoke per-section card (`section-previews.tsx`, via a `<SettingsSection preview={…}>` prop); that turned a dense page into a wall of identical demo frames and was removed. Do **not** reintroduce a `preview` prop on `<SettingsSection>` or a `section-previews` module — there is exactly one preview, the shared rail. Sections whose effect the rail can't show (Result screen, Keymap, Multiplayer) rely on their per-option chip samples (§12.2a) instead.
