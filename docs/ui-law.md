...
### 12.8 AI settings assistant — a global dock on the customise pages

The AI describer is a **global, bottom-centre dock** (`customise/_components/ai-dock.tsx` → `<CustomiseAiDock>`), mounted once in the customise layout so it rides **every** customise page — Appearance *and* Behaviour (hidden only on the full-page themes explorer). There is no dedicated `/ai` page and no inline bar. At rest it's a fully-rounded "Design with AI" pill pinned bottom-centre (a coral `Wand2` + label, a coral dot when there are unsaved AI changes); clicking it expands a `rounded-2xl` panel upward with a change log + the reusable **`<PromptInput>`** (`src/components/ui/prompt-input.tsx`: auto-growing textarea + coral send button, Enter sends). It posts to `backend.appearance.aiSuggest({ prompt, current })` (server route `src/server/routes/appearance/`, OpenRouter via `src/server/openrouter.ts`, gated `requireAuth` + a tight rate limit). The model's JSON is **never trusted directly** — `sanitize.ts` whitelists it into an `AppearancePatch` (known theme vars / appearance enums / background fields / **behaviour** fields only; CSS-injection and out-of-range values dropped).

The flow is **apply-live, then keep or undo** — the customise page *is* the preview, so changes show on the real settings + their per-section previews immediately:
- Each send applies the returned patch straight onto the live stores via `useApplyPatch` (`customise/_components/use-apply-patch.ts`), which writes theme / appearance / background / **behaviour** through the same typed setters the manual controls use, and **accumulates an undo snapshot** across the session.
- The panel **logs what changed** (per-turn summary + a label→value changes list), so the user can see each edit.
- **Apply changes** = `commit()` (keep, forget the undo point); **Undo** = `revert()` (roll the whole AI session back). You can keep chatting between turns; each new request reads the now-updated live state (`use-current-settings.ts`), so refinements naturally build up — no separate pending/merge bookkeeping.
