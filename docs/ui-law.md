...
The flow is **preview-only until Accept** (distinct from the old inline bar's global live-apply):
- **Dock** (`ai-dock.tsx`) — a single hairline bar centred above the preview (a detached element, not an overlay): a `Sparkles` mark + one input (Enter sends) + a `Suggest` button, and — once a look is proposed — a one-line summary with `Accept` / `Discard`. `needsAuth` / `empty` / `error` render as a compact muted/destructive line under the input. No message history; refining replaces the live suggestion.
