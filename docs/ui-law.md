...
### 12.8 AI settings assistant — its own page (`/customise/appearance/ai`)

The AI describer lives on a **dedicated page**, not an inline bar. The appearance page replaces the old `<AiBar>` with a compact "Design with AI" link card (coral `Sparkles`, one-line blurb, "Open") that routes to `/customise/appearance/ai`. The page (`appearance/ai/page.tsx` → `<AiCustomiser>`) stacks two **detached** elements: a minimal AI dock on top, and a **native** live preview below it — no chat log, no side panel, the dock and preview are separate (the dock does *not* float over the preview). It posts to `backend.appearance.aiSuggest({ prompt })` (server route `src/server/routes/appearance/`, OpenRouter via `src/server/openrouter.ts`, gated `requireAuth` + a tight rate limit because each call hits a paid model). The model's JSON is **never trusted directly** — `sanitize.ts` whitelists it into an `AppearancePatch` (known theme vars / appearance enums / background fields only; CSS-injection and out-of-range values dropped).

The flow is **preview-only until Apply** (distinct from the old inline bar's global live-apply):
- **Dock** (`ai-dock.tsx`) — a single hairline bar centred above the preview (a detached element, not an overlay). It hosts the reusable **`<PromptInput>`** (`src/components/ui/prompt-input.tsx`): an auto-growing textarea + a coral send button (Enter sends, Shift+Enter newline) — **no icon-mark, no "Suggest" button**. You can keep talking to it: each send **refines** the live preview and **accumulates** (the request is reasoned from base + the pending patch, via `overlayCurrent`, and the result is `mergePatch`-ed onto the pending patch — see `patch-merge.ts`). Once there are pending changes, a one-line summary appears with **`Apply changes`** (commit) and **`Reset`** (drop). `needsAuth` / `empty` / `error` render as a compact muted/destructive line under the input. No message history.
- **Preview** (`ai-preview.tsx`) — a **native** mirror of the real typing surface: the real `<Readouts>` / `<Passage>` / `<Keyboard>` at their real sizes and fonts (no shrink-to-fit, no mock chrome), composed in the same shape as `<TypingSurface>` (readouts in the passage `above` slot, passage fills, keyboard at the bottom) and simply filling the pane. It is themed with the **proposed** look *in isolation*: the appearance patch flows through `<PrefsOverrideProvider>` (the live-spectate mechanism §17.6), the theme-var patch is set **inline** on the preview container (unset vars inherit the user's current cascade), and the background patch paints behind it. Nothing touches the global stores, so the dock + the rest of the app keep the user's current theme and stay readable no matter how wild the suggestion is.
- **Apply changes** commits the accumulated pending patch to the global stores via `useApplyPatch` (`apply` + `commit`); **Reset** just drops it (the preview falls back to the live state). The 5 `html`-driven surface/chrome knobs (`cardSurfaces`, `dividers`, `pagePadding`, `backgroundFill`, `monochromeChrome`) can't be scoped to the preview container (their globals.css rules key off `html[data-ft-*]`), so they preview as no-ops and only take visible effect on Accept; everything else (colours, fonts, radius, passage tokens, caret-idle, mistakes, tape, highlight, lines, width) previews 1:1.

**Current-state context, delta output, and tools (server).** The client sends a compact snapshot of the user's **current** settings with each request (`use-current-settings.ts` → `aiSuggest({ prompt, current })`): the resolved theme vars (via `getComputedStyle`), the primitive appearance prefs, and the background knobs (never the imageUrl — it may be a multi-MB data URL). `prompt.ts` → `buildUserMessage` trims that to the AI-touchable fields and prepends it, so the model resolves abstract/relative asks ("warmer", "a bit bigger", "match the accent") against real values and replies with **only the delta** (not the full settings object — saves tokens). The model may also invoke server-side **tools** (`tools.ts`, resolved deterministically before sanitizing, so the security boundary is unchanged): `palette` (build a coherent OKLCH colour set from a base — a colour token or a current var — with light/dark detected from the current background; `color.ts` owns the maths) and `randomize` (a tasteful random valid value per named field). Direct model values win over tool fills on a key conflict. Adding a tool means a resolver in `tools.ts` + a line in `prompt.ts` + tests — never trusting raw model output.

Rules:
- The AI only ever writes through the **same typed store setters** the manual controls use (`setVar`, appearance/background `update`), and only on **Accept** — never a parallel write path, never a global write during preview.
- The server whitelist is the security boundary. Widen it deliberately (add the key + validator in `sanitize.ts` and the matching line in `prompt.ts`), never by trusting the model. Tool output (`tools.ts`) is merged into the patch and **re-sanitized** like everything else.
...
- The page fits the customise content area with no page scroll (height keyed to the viewport so the whole preview stays on screen); the dock floats and never adds height.
