...
The AI describer lives on a **dedicated page**, not an inline bar. The appearance page replaces the old `<AiBar>` with a compact "Design with AI" link card (coral `Sparkles`, one-line blurb, "Open") that routes to `/customise/appearance/ai`. The page (`appearance/ai/page.tsx` → `<AiCustomiser>`) is a **two-pane, no-scroll surface**: a **chat panel** on the left (desktop) / bottom (mobile), and a **full live preview** on the right / top. It posts to `backend.appearance.aiSuggest({ prompt })` (server route `src/server/routes/appearance/`, OpenRouter via `src/server/openrouter.ts`, gated `requireAuth` + a tight rate limit because each call hits a paid model). The model's JSON is **never trusted directly** — `sanitize.ts` whitelists it into an `AppearancePatch` (known theme vars / appearance enums / background fields only; CSS-injection and out-of-range values dropped).

The flow is **preview-only until Accept** (distinct from the old inline bar's global live-apply):
- **Chat** (`ai-chat.tsx`) — a message log (user prompts + assistant summary/changes), a shimmer "Designing your look" thinking line (`thinking.tsx`, the sanctioned loader motion §13, `motion-safe`), an Accept/Discard bar while a suggestion is live, and a textarea (Enter sends, Shift+Enter newline).
- **Preview** (`ai-preview.tsx`) — a read-only mirror of the real typing surface (chrome bands + real `<Readouts>` / `<Passage>` / `<Keyboard>`) themed with the **proposed** look *in isolation*: the appearance patch flows through `<PrefsOverrideProvider>` (the live-spectate mechanism §17.6), the theme-var patch is set **inline** on the preview container (unset vars inherit the user's current cascade), and the background patch paints behind it. Nothing touches the global stores, so the chat + the rest of the app keep the user's current theme and stay readable no matter how wild the suggestion is. The surface is composed at a fixed design size and shrink-to-fit scaled by `<FitBox>` so the whole thing is visible with **no scroll**.
...
- No API key configured → the route returns a clear error the chat surfaces as an assistant message; unauthenticated → the chat prompts sign-in. Both are handled states, not crashes.
- The page fits the customise content area with no page scroll (height keyed to the viewport so the chat input + the whole preview stay on screen); only the chat message log scrolls internally.
