...
3. **Setting rows** — one per option, rendered via `<SettingsRow label="…" control={…} />` from `customise/_components/row.tsx`. Each row has the label on the left and a right-aligned control. **Rows have no fill** — they are hairline-outlined containers (`rounded-md border border-border`, no background) sitting on the `bg-background` page, so the row itself recedes and the *control* carries the surface. Chips *inside* a row are **`bg-muted`** (the surface that used to sit on the row now lives on the option you actually click), with `border-border` for definition; their hover routes through `bg-accent` (§2.2) and the active state inverts to the coral `bg-primary/10 border-primary text-primary`. `min-h-16` baseline, `max-h-48` (3×) cap. The Card-based rich rows (`<ColorRow>` desktop, `<FontRow>` desktop) carry `bg-transparent` so every option surface matches (the shadcn `Card` default `bg-card` is overridden off). *Rationale: once the customise content panel was removed (the page is flush `bg-background`), the old `bg-muted` row band read as a stray dark stripe; moving the surface onto the control is what makes the list legible on the flush page.*
...
**Current-state context, delta output, and tools (server).** The client sends a compact snapshot of the user's **current** settings with each request (`use-current-settings.ts`): resolved theme vars (via `getComputedStyle`), primitive appearance + **behaviour** prefs, and the background knobs (never the imageUrl — it may be a multi-MB data URL). `prompt.ts` → `buildUserMessage` trims that to the AI-touchable fields and prepends it, so the model resolves abstract/relative asks ("warmer", "a bit bigger", "stricter", "match the accent") against real values and replies with **only the delta**. The model may also invoke server-side **tools** (`tools.ts`, resolved deterministically before sanitizing, so the security boundary is unchanged): `palette` (a coherent OKLCH set from a base — colour token or current var — light/dark detected from the current background; `color.ts` owns the maths) and `randomize` (a tasteful random valid value per named field). Direct model values win over tool fills on a key conflict.
...
- The server whitelist is the security boundary. Widen it deliberately (add the key + validator in `sanitize.ts` — `AI_THEME_VARS`, `APPEARANCE_FIELDS`, or `BEHAVIOUR_FIELDS` — and the matching line in `prompt.ts`), never by trusting the model. Tool output (`tools.ts`) is merged into the patch and **re-sanitized** like everything else.
