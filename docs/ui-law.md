...
The friends hub (`src/app/friends/_components/`) is a page about *people*, not an activity feed. It runs at the **standard wide app width** (`max-w-6xl`, like `/profile`) and uses that width with a **Live hero + two-column body**, so it doesn't read as a narrow outlier next to the rest of the app. It's the one place flinttype runs avatars at full fidelity (an explicit product call, so the rest of the app stays avatar-free). Everything is **inline — no popups, no chunky tabs, no on-page activity feed** (an early iteration had a feed-spine + a collapsed activity section; gone — don't reintroduce them).

Layout (`friends-view.tsx`):
1. **Header** — a slim `Tag` + `h1` + one-line description.
2. **`Live now`** (`presence-sections.tsx` → `LiveNow`) — the hub's headline, **full-width and deliberately big/exciting**: a two-up grid (`sm:grid-cols-2`) of `<LiveCard>`s, each a card (not a thin row) with a `lg` live avatar, handle + tags, a pulsing-coral "Typing now", the live **WPM in `text-2xl/3xl`**, a coral progress bar, and a **filled coral "Watch live" button** → `/live/<userId>`. This is the one section where coral CTAs multiply on purpose (it's the live zone). The card shows a **static glimpse only** (WPM + progress) — it never streams a live preview in the list (that would burn the poll/CPU); the real stream is the watch page. **Hidden entirely when nobody's live.**
3. **Body** — below the Live hero, a `lg:grid-cols-[1.6fr_1fr]` split: **People** (`people-panel.tsx`, the spine — §17.2 switch + search + `FriendListRow`s) on the left, and a **rail** on the right with `<ChallengesSection>` (incoming duels, inline, "Accept", → `/duel/<id>`) + `<OnlineNow>` (online-not-live avatar chips with an activity word, §17.7). On mobile the rail stacks **above** People (order utilities). The rail only earns its column when it has content (`hasRail`); otherwise People spans full width so the page never leaves a dead column.
