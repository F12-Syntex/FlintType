...
3. **`<ChallengesSection>`** (`challenges-section.tsx`) — incoming duel challenges, surfaced **inline** (`backend.duels.list().incoming`), never a link out to a `/duels` list page. A `Swords` eyebrow + count, then one row per challenge: a `Swords` tile, the challenger's handle + tags, "Dares you to beat N wpm", and a quiet **"Take it"** word. **The whole row is the link** (to `/duel/<id>`, where you actually race the ghost — that's a typing activity, legitimately its own screen); the affordance is text, not a filled coral button, so N challenges don't paint N coral sparks (same pattern as LiveNow). **Hidden when none.** Starting a challenge stays in the `<FollowButton>` ⋯ menu (Challenge → `/duel/new?opponent=<id>`, the run composer); only the *list* of waiting challenges is inline.
