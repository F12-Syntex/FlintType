...
- **Registry-driven.** Content lives in `src/lib/updates.ts` (`UPDATES`): `{ slug, version, title, tagline, date, versions[], highlights[] }`, where each highlight is `{ preview: string, title, body }` — `preview` keys into `UPDATE_PREVIEWS`. Add an entry; the route + changelog link derive from it. No per-card bespoke page.
- **Bespoke previews, not icons.** Each highlight renders a small **mini-mockup** of the feature (hub / challenges / spectate / profile) from `UPDATE_PREVIEWS` (`src/app/updates/_components/previews.tsx`) — little framed UI snapshots (a people list, a duel row, a live passage, a profile card) built **entirely from the fixed `ft-*` light tokens** so they stay light inside the forced-light card. They're `aria-hidden` decoration; the title + blurb carry the meaning. A generic lucide-icon tile is **not** good enough here — the card is an advert, the previews sell the feature.
- **The card** (`src/app/updates/[slug]/page.tsx`) is a **forced-light** asset — fixed `ft-*` paper-and-ink tokens (§2.3) so it renders the same in dark mode and screenshots as a clean light promo. **No `<AppChrome>`** — the page *is* the card, centred on a `bg-ft-paper-soft` field with a small "Back to changelog" link beneath. Shape: brand `<Logo>` + ember version pill, a big `text-4xl/5xl` headline + tagline, then the **preview feature grid** (each cell a framed preview + title + one-line body), then a brand footer strip. Stay within the hairline/no-glow brand (no drop shadows).
