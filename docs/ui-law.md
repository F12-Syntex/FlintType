...
Friend lists (Friends / Following / Followers) switch via a segmented control built from the **exact idiom as the TopBar main nav** (`top-bar.tsx`), so every switch in the app reads as one family: a `rounded-md border border-border bg-card p-0.5` container, each segment `rounded-[5px] px-3 py-1.5 text-[13px] tracking-tight` with a subtle `bg-foreground/[0.06] font-semibold text-foreground` active tint and a `text-muted-foreground hover:bg-foreground/[0.04] hover:text-foreground` rest state. Counts ride inline per segment in `tabular-nums text-muted-foreground`. Full-width (segments `flex-1`) on mobile so it never overflows 375px; hugging (`sm:w-fit`, `sm:flex-initial`) from `sm+` like the nav. **It shares one row with the search input** from `sm+` (switch hugging left, search `flex-1`); stacked on mobile. Default to Friends. This is the **one** segmented-switch style — do **not** use the old chunky `bg-muted/40` track (that idiom is retired across the app; the duels page is the last holdout and should migrate when next touched).
...
- **`<SpectatePill enabled>`** — presentational. `rounded-md` hairline chip, mono `text-[10px]` uppercase tracked, with a leading status dot + an `Eye` glyph (lucide). The single coral spark is the dot in the **on** state (`bg-primary` with `motion-safe:animate-pulse`); the off state is quiet ink (`text-muted-foreground`, `bg-card`). Pulsing coral dots are reserved for live-spectate state — this consent chip plus the live markers in the hub + watch surfaces (§17.5). They convey live state, not decoration (§13), and every one is gated behind `motion-safe:` so reduced-motion users get a static dot. Don't introduce a pulsing dot anywhere else.
...
The friends hub (`src/app/friends/_components/`) is a **calm, single-column page** (`max-w-2xl`, centred) about *people*, not an activity feed. It's the one place flinttype runs avatars at full fidelity (an explicit product call, so the rest of the app stays avatar-free). Everything is **inline — no popups, no chunky tabs, no two-column rail, no on-page activity feed** (earlier iterations had a feed-spine + People rail + a collapsed activity section; all gone — don't reintroduce them). The single column scales straight from 375px to desktop.

Top to bottom (`friends-view.tsx`):
1. **Header** — a slim `Tag` + `h1` + one-line description.
2. **`Live now`** (`presence-sections.tsx` → `LiveNow`) — mutual friends currently broadcasting (`live.friendsLive`). Eyebrow dot is the pulsing coral spark. Each row is the **watch trigger** (whole row links to `/live/<userId>`, hover `bg-accent`): avatar (`status="live"`), handle + tags, a coral mini progress bar + live WPM, and a plain **"Watch"** word (no icon — the row itself is the affordance). The CTA is the row, not a filled button, so N live friends don't paint N coral sparks (§17.1). **Hidden entirely when nobody's live** — no dead "nobody's here" card.
3. **`<ChallengesSection>`** (`challenges-section.tsx`) — incoming duel challenges, surfaced **inline** (`backend.duels.list().incoming`), never a link out to a `/duels` list page. A `Swords` eyebrow + count, then one row per challenge: a `Swords` tile, the challenger's handle + tags, "Dares you to beat N wpm", and a quiet **"Accept"** word. **The whole row is the link** (to `/duel/<id>`, where you actually race the ghost — that's a typing activity, legitimately its own screen); the affordance is text, not a filled coral button, so N challenges don't paint N coral sparks (same pattern as LiveNow). **Hidden when none.** Starting a challenge stays in the `<FollowButton>` ⋯ menu (Challenge → `/duel/new?opponent=<id>`, the run composer); only the *list* of waiting challenges is inline.
...
5. **People** (`people-panel.tsx`) — the spine: the §17.2 segmented switch (Friends / Following / Followers) + search + `FriendListRow`s with the full `<FollowButton compact>` management.

**Activity stays out of this page.** Friends' personal bests, new follows, mutual-friendship and duel events live **only** in the notifications bell (`notifications-popover.tsx`, fed by `notifications.list()`). The hub is for *who's here and who you can play*, not a timeline; don't re-add an on-page feed.
...
3. **Stats + skill radar** — the stats strip (`grid-cols-2 sm:grid-cols-4`: Tests / Time / Best WPM / Streak) and a **compact skill radar** share one row (`md:grid-cols-[minmax(0,1fr)_auto]`): stats on the left, the radar filling the otherwise-empty right edge on `md+`, stacking under the stats on mobile. No top border (the experience bar divides). Stat values stay ink on a visitor's view so the coral Follow CTA is the page's one spark (§2); the owner's own profile (no follow button) may accent them coral. The radar is omitted entirely until there's at least one completed test.

### 18.2 Skill radar (`<SkillRadar>`, `src/components/skill-radar.tsx`)

A **four-spoke** radar (**Speed, Accuracy, Consistency, Endurance**) — the shape is the story; there's no value table beside it (redundant; raw numbers live in the stats strip + Personal bests). It lives **inside the hero** (§18.1, `compact` sizing) so it doesn't waste a whole section. Profile values are derived by `deriveSkills` (`derive-stats.ts`) — each spoke a 0–100 score: Speed = best WPM vs a **300** ceiling; Endurance = best WPM on any ~30s+ run (keyed off actual elapsed time ≥ ~28s, since `mode` doesn't distinguish time vs words) vs a **250** ceiling; Accuracy rescaled into the 80–100 band; Consistency = 100 − WPM CV.
