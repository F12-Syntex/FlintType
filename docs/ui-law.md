...
The flow is **preview-first**: when a suggestion returns, the patch is applied straight onto the live prefs stores via `useApplyPatch` (so the whole page + every per-section preview repaint 1:1 — the previews *are* the real state), and a suggestion card reveals (§13) with a one-line summary + a changed-rows list. **Accept** keeps it; **Discard** reverts from the snapshot `useApplyPatch` captured before applying.
