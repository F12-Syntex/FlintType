...
- **Live-session store is DB-backed** (`live_sessions` table, migration `0011`; `liveSessionsRepo`), exactly like presence — one row per broadcaster, upserted on an **adaptive cadence** (`broadcastPlan` in `src/lib/live-cadence.ts`): `WATCHED_MS` (700ms) while a spectator is attached, a slow `HEARTBEAT_MS` (2s) heartbeat while typing-but-unwatched (discoverability), and it **stops entirely** when resting (not begun) or once an *unwatched* results screen passes `DONE_BROADCAST_GRACE_MS` (30s) — so an idle results tab can't heartbeat forever. Chosen over an in-memory authority map because a live session keys on the authenticated Clerk userId (which the authority can't see) and a shared table is **correct across Vercel instances**: a spectator poll routed to a different instance than the broadcaster's push still finds the snapshot. A snapshot is "live" only while fresh (`LIVE_TTL_MS`, 6s); a broadcaster that stops pushing ages out at read time with no goodbye.
...
- **Cost of v1:** the upsert is one row write per active broadcaster on the adaptive cadence above (700ms watched / 2s unwatched-typing / stops when resting or 30s after an unwatched finish), and the spectator poll is one indexed read per active viewer. Three controls keep the meters honest: (1) the client loops — broadcaster, spectator (`useBufferedWatch`), and the `/friends` hub poll — all **pause on a hidden tab** (zero requests when backgrounded); (2) `resolveUserDisplays` (`src/server/user-display.ts`) caches the Clerk display projection for 30s, so a ~1Hz `live.watch` poll resolves the (static) subject identity from cache instead of a Clerk round-trip per poll — important because Vercel bills on function *duration* and a Clerk wait dominates the hot path; (3) the done-grace cap above. Fine at the current scale; the SSE upgrade removes both meters when it's worth it.
