...
- **Broadcasting is now ambient (no `/live` broadcaster page).** A `PracticeLiveBroadcast` mounted inside `PracticeProvider` streams from the real practice + sudden-death surfaces while the user has opted in (Customise → Behaviour → Live spectating); the snapshot is windowed to `LIVE_MAX_WORDS` so a runaway TIME buffer can't blow the wire cap. Watching is reached from the reworked `/friends` hub. See `docs/ui-law.md` §17.4 (consent chip) + §17.5 (hub).
